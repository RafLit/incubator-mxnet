...
void SgMKLDNNFCOp::Forward(const OpContext& ctx, const std::vector<NDArray>& in_data,
...
static inline void FillBaseInputOutputInfo(const FullyConnectedParam& param,
                                           ...
                                           std::vector<T>* base_out_attrs, std::vector<T>* in_attrs,
...
static bool SgMKLDNNFCInferShape(const nnvm::NodeAttrs& attrs, mxnet::ShapeVector* in_shapes,
  ...
  if (full_param.mkldnn_param.quantized) {
    ...
    FillBaseInputOutputInfo(full_param.default_param, &base_in_shapes, &base_out_shapes, in_shapes,
                            out_shapes);
...
static bool SgMKLDNNFCStorageType(const nnvm::NodeAttrs& attrs, const int dev_mask,
  ...
  if (full_param.mkldnn_param.quantized) {
    ...
    FillBaseInputOutputInfo(full_param.default_param, &base_in_attrs, &base_out_attrs, in_attrs,
                            out_attrs);
...
static OpStatePtr CreateSgMKLDNNFCState(const nnvm::NodeAttrs& attrs, Context ctx,
