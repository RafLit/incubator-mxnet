...
bool SupportMKLDNNNumpyTranspose(const NumpyTransposeParam& param, const NDArray& data) {
  auto data_ndim = data.shape().ndim();
...
typedef ParamOpSign<NumpyTransposeParam> MKLDNNNumpyTransposeSignature;

class MKLDNNNumpyTransposeForward {
 public:
  std::shared_ptr<mkldnn::memory> data_;
  std::shared_ptr<mkldnn::memory> out_;
  std::shared_ptr<mkldnn::memory::desc> dst_md_;
 ...
 public:
  MKLDNNNumpyTransposeForward(const NumpyTransposeParam& param, const NDArray& data) {
    auto shape     = data.shape();
    auto data_ndim = shape.ndim();
    auto axes_ndim = param.axes.ndim();
    auto axes      = mxnet::TShape(data_ndim, -1);
...
static MKLDNNNumpyTransposeForward& GetNumpyTransposeForward(const NumpyTransposeParam& param,
                                                   const NDArray& data) {
#if DMLC_CXX11_THREAD_LOCAL
  static thread_local std::unordered_map<MKLDNNNumpyTransposeSignature, MKLDNNNumpyTransposeForward, OpHash>
      fwds;
#else
  static MX_THREAD_LOCAL
      std::unordered_map<MKLDNNNumpyTransposeSignature, MKLDNNNumpyTransposeForward, OpHash>
          fwds;
#endif
  MKLDNNNumpyTransposeSignature key(param);
  key.AddSign(data);

  auto it = fwds.find(key);
  if (it == fwds.end()) {
    MKLDNNNumpyTransposeForward fwd(param, data);
    it = AddToCache(&fwds, key, fwd);
  }
  return it->second;
}
...
void MKLDNNNumpyTransposeForward(const nnvm::NodeAttrs& attrs,
  ...
  auto fwd = GetNumpyTransposeForward(param, data);
  fwd.SetNewMem(data, output);
  fwd.Execute();
}
}  // namespace op
